...
function dtab(tab, el) {
  ...
  if (tab === 'info') {
    ...
    body.innerHTML=`
      ...
      <div class="form-group"><label class="form-label">미리보기</label><div id="s-prev" style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px 12px;font-size:13px;min-height:32px;color:var(--text1)"></div></div>
    ...
    DB('sync_logs').select('*').eq('room_id', r.id).order('synced_at',{ascending:false}).limit(20)
      .then(({data}) => {
        document.getElementById('r-log').innerHTML = !data?.length ? '<div style="color:var(--text2);font-size:13px;padding:.5rem">이력 없음</div>' :
