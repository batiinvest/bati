  ...
  if (tab === 'info') {
    body.innerHTML = `
      <div style="display:flex;flex-direction:column;gap:2px;margin-bottom:1rem">
        ${[['이름',escapeHtml(r.name)],['종목코드',escapeHtml(r.code||'-')],['산업',escapeHtml(r.cat)],['세부 분야',escapeHtml(r.sub_cat||'-')],['멤버',`${(r.members||0).toLocaleString()} / ${r.max_members||1000}`],['상태',r.status==='full'?'<span class="badge badge-full">정원 마감</span>':'<span class="badge badge-open">입장 가능</span>'],['Chat ID',`<code style="font-size:calc(11px*var(--m-label));background:var(--bg3);padding:2px 6px;border-radius:4px">${escapeHtml(r.chat_id)}</code>`],['링크',safeUrl(r.link)?`<a href="${escAttr(safeUrl(r.link))}" target="_blank" rel="noopener">${escapeHtml(r.link.slice(0,45))}</a>`:'-'],['키워드',r.keywords?`<span style="font-size:calc(12px*var(--m-sub));color:var(--text1)">${escapeHtml(r.keywords)}</span>`:'-']].map(([k,v])=>`<div style="display:flex;justify-content:space-between;align-items:center;padding:7px 0;border-bottom:1px solid var(--border);font-size:calc(13px*var(--m-body))"><span style="color:var(--text1)">${k}</span><span>${v}</span></div>`).join('')}
      </div>
    ...
    if (!canEdit()) { body.innerHTML='<div style="padding:1rem;color:var(--text2);font-size:calc(13px*var(--m-body));text-align:center">수정 권한이 없습니다.</div>'; return; }
    ...
    if (!canEdit()) { body.innerHTML='<div style="padding:1rem;color:var(--text2);font-size:calc(13px*var(--m-body));text-align:center">발송 권한이 없습니다.</div>'; return; }
    body.innerHTML=`
      ...
      <div class="form-group"><label class="form-label">미리보기</label><div id="s-prev" style="background:var(--bg3);border:1px solid var(--border);border-radius:var(--radius-sm);padding:8px 12px;font-size:calc(13px*var(--m-body));min-height:32px;color:var(--text1)"></div></div>
    ...
    DB('sync_logs').select('*').eq('room_id', r.id).order('synced_at',{ascending:false}).limit(20)
      .then(({data}) => {
        document.getElementById('r-log').innerHTML = !data?.length ? '<div style="color:var(--text2);font-size:calc(13px*var(--m-body));padding:.5rem">이력 없음</div>' :
...
//  HELPERS
// ══════════════════════════════════════════
const prev = (v, id) => { const el = document.getElementById(id); if (el) el.innerHTML = v || '<span style="color:var(--text2)">미리보기</span>'; };
