...
function _tempReasonLine(parts, score, color) {
  ...
  const chips = picks.map(p =>
    `<span style="font-size:calc(11px*var(--m-label));color:${color};background:${color}1a;border-radius:3px;padding:1px 6px;white-space:nowrap">${p.label}</span>`
  ).join(' ');
  return `<div style="display:flex;flex-wrap:wrap;gap:4px;align-items:center;margin-bottom:6px">
    <span style="font-size:calc(11px*var(--m-label));color:var(--text2);font-weight:600">${word}</span>${chips}</div>`;
}
...
async function renderMarketTemperature() {
  ...
  if (prev !== null) {
    ...
    diffBadge = `<span style="font-size:calc(11px*var(--m-label));font-weight:600;color:${diffColor};
      margin-left:6px">${diffStr}</span>
      <span style="font-size:calc(11px*var(--m-label));color:var(--text2);margin-left:4px">(전일 ${prev.score}점)</span>`;
  }
  ...
  let streakBadge = '';
  if (streak >= 2) {
    streakBadge = `<span style="font-size:calc(11px*var(--m-label));font-weight:600;color:var(--text2);
    ...
    streakBadge = `<span style="font-size:calc(11px*var(--m-label));font-weight:700;color:${t.gradeColor};
  ...
  <div style="display:flex;align-items:center;gap:16px;margin-bottom:10px">
    ...
    <div style="text-align:center;min-width:54px;flex-shrink:0">
      <div style="font-size:calc(36px*var(--m-title));font-weight:800;line-height:1;color:${t.gradeColor};
        font-variant-numeric:tabular-nums">${t.score}</div>
      <div style="font-size:calc(11px*var(--m-label));color:var(--text2);margin-top:2px">/ 100</div>
    </div>

    <!-- 게이지 + 등급 -->
    <div style="flex:1">
      <div style="font-size:calc(14px*var(--m-body));font-weight:700;color:${t.gradeColor};
        margin-bottom:4px;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
        ${t.gradeEmoji} ${t.gradeTxt}
        ${streakBadge}
        ${diffBadge}
        ${t.crashNote ? `<span style="font-size:calc(11px*var(--m-label));font-weight:600;color:#f5365c;
      ...
      <div style="display:flex;justify-content:space-between;
        font-size:calc(11px*var(--m-label));color:var(--text2);margin-top:3px;padding:0 1px">
  ...
  if (ev) {
    const factors = t.parts.map(p => {
      ...
      <div class="temp-detail-row">
        <span style="min-width:140px;color:var(--text1);font-size:calc(11px*var(--m-label))">${p.label}${p.hint ? ` <span style="color:var(--text2);font-size:calc(11px*var(--m-label))">${p.hint}</span>` : ''}</span>
        <div class="temp-detail-bar">
          <div class="temp-detail-fill" style="width:${pct}%;background:${barColor}"></div>
        </div>
        <span style="min-width:34px;text-align:right;font-size:calc(11px*var(--m-label));font-weight:600;color:var(--text1)">${p.pts}<span style="color:var(--text2);font-weight:400">/${p.max}</span></span>
      </div>`;
    }).join('');
    ev.innerHTML = `
      <div onclick="toggleMjEvidence()" style="cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 1rem;font-size:calc(11px*var(--m-label));color:var(--text2)"
        ...
        <span style="font-size:calc(11px*var(--m-label))">S&amp;P·환율·5일추세·상승종목·수급·VIX·금리 7지표 (점수 산출 기준)</span>
        <span id="mj-ev-toggle" style="margin-left:auto;font-size:calc(11px*var(--m-label))">펼치기 ▾</span>
      </div>
